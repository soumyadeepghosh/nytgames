...
bool SudokuSolver::processCell(char inputChar, unsigned row, unsigned col) {
    ...
    if (num < 1 || num > GRID_SIZE) {
        std::cerr << " Incorrect input value " << inputChar << " found." <<
            " All sudoku cell values must be between 1 and " << GRID_SIZE << std::endl;
        return false;
    }
...
bool SudokuSolver::processInputFile(const char* inputFileName) {
    ...
    while (inputFile >> c) {
        if (currentRow > GRID_SIZE) {
            saneConfig = true;
            std::cerr << " Ignoring everything after the first " << GRID_SIZE << " lines." <<
                std::endl;
            return true;
        }
        ...
        if (dbgFunction) {
            std::cout << " " << currentRow << ", " << currentCol << " : " << c << ", " <<
                sudokuState[getIndex(currentRow, currentCol)] << std::endl;
        }
        ++currentCol;
...
SudokuTransaction::SudokuTransaction(const array<SudokuValue, GRID_SIZE * GRID_SIZE>& input) {
    ...
    for (unsigned i = 0; i < GRID_SIZE; i++) {
        for (unsigned j = 0; j < GRID_SIZE; j++) {
            ...
            // If the state of the sudoku is not solvable, then return prematurely for this
            // transaction.
            ...
            if (dbgMethod) {
                std::cout << "    New rows[" << i << "] : " << rows[i] << std::endl;
                std::cout << "    New cols[" << j << "] : " << cols[j] << std::endl;
                std::cout << "    New sqrs[" << sqIndex << "] : " << squares[sqIndex] << std::endl;
            }
        }
    }
    if (solved) {
    ...
    dbgMethod = false;
    std::cout << " Initial state formed. Determining allowed values." << std::endl;
...
SudokuTransaction::SudokuTransaction(const SudokuTransaction& parent,
    ...
    copyState(parent);
    validTransaction = true;
    std::cout << "New TX setting (" << row << ", " << col << ") to " << value << std::endl;
    setCell(getIndex(row, col), value);
    if (solved || !validTransaction) return;
    // Update all single possibility states that emerged. This prevents unnecessary forking off of
    // transactions.
    bool dbgMethod = true;
    validTransaction &= updateSinglePossibilities(dbgMethod);
    if (solved || !validTransaction) return;
...
void SudokuTransaction::reverseIndexLookup(unsigned index, unsigned* row, unsigned* col) const {
    *row = index / GRID_SIZE;
    *col = index % GRID_SIZE;
}

void SudokuTransaction::setCell(unsigned index, SudokuValue value, bool dbgMethod) {
    // Update the entry at index = getIndex(row, col) to value.
    unsigned row, col;
    reverseIndexLookup(index, &row, &col);
    if (sudokuState.at(index) != -1) {
        std::stringstream ss;
        ss << "Trying to set value of cell (" << row << ", " << col << ") already containing " <<
            sudokuState.at(index) << " to " << value << std::endl;
    ...
    for (unsigned i = 0; i < GRID_SIZE; i++) {
        for (unsigned j = 0; j < GRID_SIZE; j++) {
            ...
            for (unsigned k = 1; k <= GRID_SIZE; k++) {
                ...
                bool candidatePossible = isCandidatePossible(candidateValue, i, j, sqIndex);
                if (!candidatePossible) continue;
                allowedState[index] |= candidateValue;
                possibilities[index]++;
                foundPossibilities = true;
                if (dbgMethod)
                    std::cout << "        Allowed at (" << i << ", " << j << ") : " << k << " " <<
                        candidateValue << " " << allowedState[index] << " " << possibilities[index]
                        << std::endl;
            }
            if (possibilities[index] == 0) {
                if (dbgMethod)
                    std::cout << "    Allowed not found for cell (" << i << ", " << j << ")" << std::endl;
                return false;
            } else if (possibilities[index] == 1) {
                std::cout << "    Found single allowed for (" << i << ", " << j << ") : ";
...
unsigned SudokuTransaction::getSquareIndex(unsigned row, unsigned col) const {
...
bool SudokuTransaction::isCandidatePossible(unsigned candidateValue,
    ...
    if (dbgMethod) {
        ...
        std::cout << "    sqrs[" << sqIndex << "] : " << squares[sqIndex] << std::endl;
    ...
    if (squares[sqIndex] != 0 && ((squares[sqIndex] & candidateValue) != 0)) return false;
    return true;
}
...
unsigned SudokuTransaction::getNextCellToFill(bool dbgMethod) const {
    ...
    if (dbgMethod) {
        ...
        std::cout << "    Next cell to fill: (" << row << ", " << col << ")" << std::endl;
    }
    return rv;
}

bool SudokuTransaction::solve(bool dbgMethod) {
    // This is called after all the single possibility cells are filled out, and there is at least
    // one unfilled cell in the puzzle with multiple possibilities.
    ...
    // If there are no possibilities, then clearly we cannot solve this puzzle.
    if (values.size() == 0) return false;
    for (auto value : values) {
        std::unique_ptr<SudokuTransaction> nextStep =
            std::make_unique<SudokuTransaction>(*this, nextCellToFill, value);
...
SudokuValue SudokuTransaction::getSinglePossibility(unsigned row, unsigned col) const {
    ...
    if (possibilities.size() != 1) {
        std::stringstream ss;
        ss << "Discrepancy between possibilities and actual possible values for ("
            << row << ", " << col << ")";
        throw std::runtime_error(ss.str());
    }
...
const vector<SudokuValue> SudokuTransaction::getPossibilities(unsigned index) const {
    unsigned row, col;
    reverseIndexLookup(index, &row, &col);
    return getPossibilities(row, col);
}

const vector<SudokuValue> SudokuTransaction::getPossibilities(unsigned row, unsigned col) const {
    ...
    do {
        for (unsigned i = 0; i < possibilities.size(); i++) {
            if (possibilities.at(i) == 1) {
                ...
                if (dbgMethod)
                    std::cout << "Single possibility for index " << i << " : " << getSinglePossibility(i) << std::endl;
            }
        }
        ...
        if (dbgMethod) {
            ...
            std::cout << " Setting (" << row << ", " << col << ") to " << nextStep.getValue() << std::endl;
        }
